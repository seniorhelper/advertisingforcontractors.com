...
var S = { trade:null, ticket:0, margin:0, close:0, step:0, turns:0, misses:0,
          seen:{}, lastId:null, spin:0, mood:'main' };
...
function calcFlow(raw, say, mood){
...
function Cine(){
  ...
  + '<div class="afcb-hero">' + FULL('main') + '</div>'
...
function respond(raw, api){
  ...
  if (S.step > 0 && S.step < 4){ if (calcFlow(raw, say, api.mood)) return; }
  ...
  if (hit){
    ...
    if (api.mood) api.mood(MOOD_OF[hit.id] || 'main');
...
/* which face he wears for which subject — anything unlisted falls back to main */
var MOOD_OF = {
  pricing:'shades', cheap:'shades', vip:'shades', goodcpl:'shades', roi:'shades',
  website:'shades', contract:'straight', whyyou:'straight', seotime:'straight',
  agency_vs:'straight', buyvsown:'straight', angi:'straight', burned:'straight',
  diy:'straight', results:'straight', joke:'grin', quote:'grin', thanks:'grin',
  haha:'grin', love:'grin', scale:'grin', whyfree:'grin', freebot:'grin'
};

function headTag(cls, mood){
  return '<img class="' + cls + '" src="' + MOODS[mood || 'main'] + '" alt="" draggable="false">';
}

/* Full mascot: oversized photo head bobbling on the tape-measure body. */
function FULL(mood){
  var src = MOODS[mood || 'main'];
  return '<svg viewBox="0 0 720 1240" aria-hidden="true" focusable="false">'
  + '<defs>'
  + '<linearGradient id="afcCase" x1="0" y1="0" x2="1" y2="1">'
  +   '<stop offset="0%" stop-color="#F9D64C"/><stop offset="52%" stop-color="#F0BE22"/>'
  +   '<stop offset="100%" stop-color="#D29814"/></linearGradient>'
  + '<linearGradient id="afcChr" x1="0" y1="0" x2="0" y2="1">'
  +   '<stop offset="0%" stop-color="#EDF1F5"/><stop offset="100%" stop-color="#6E7885"/></linearGradient>'
  + '<linearGradient id="afcLth" x1="0" y1="0" x2="0" y2="1">'
  +   '<stop offset="0%" stop-color="#A3714A"/><stop offset="100%" stop-color="#5E3D23"/></linearGradient>'
  + '</defs>'
  + '<ellipse cx="360" cy="1196" rx="168" ry="20" fill="#000" opacity=".34"/>'
  /* boots */
  + '<path d="M392 1074 h56 a20 20 0 0 1 20 20 v10 h-76 z" fill="#7A5232" stroke="#14181F" stroke-width="6" stroke-linejoin="round"/>'
  + '<rect x="386" y="1098" width="92" height="22" rx="11" fill="#20262E" stroke="#14181F" stroke-width="5"/>'
  + '<path d="M248 1074 h60 v30 h-80 v-10 a20 20 0 0 1 20 -20 z" fill="#8B5E3C" stroke="#14181F" stroke-width="6" stroke-linejoin="round"/>'
  + '<rect x="222" y="1098" width="94" height="22" rx="11" fill="#20262E" stroke="#14181F" stroke-width="5"/>'
  /* legs + knee pads */
  + '<rect x="322" y="1000" width="20" height="80" rx="10" fill="#14181F"/>'
  + '<rect x="380" y="1000" width="20" height="80" rx="10" fill="#14181F"/>'
  + '<rect x="310" y="1024" width="42" height="34" rx="13" fill="#2C333D" stroke="#14181F" stroke-width="5"/>'
  + '<rect x="370" y="1024" width="42" height="34" rx="13" fill="#2C333D" stroke="#14181F" stroke-width="5"/>'
  /* tape case */
  + '<rect x="266" y="832" width="188" height="186" rx="32" fill="#171C24"/>'
  + '<rect x="280" y="846" width="160" height="158" rx="24" fill="url(#afcCase)" stroke="#14181F" stroke-width="5"/>'
  + '<rect x="314" y="828" width="92" height="20" rx="8" fill="#0C0F14"/>'
  + '<rect x="290" y="872" width="36" height="46" rx="11" fill="#1C222B" stroke="#14181F" stroke-width="4"/>'
  + '<rect x="297" y="879" width="22" height="22" rx="7" fill="#A6CE39"/>'
  + '<g transform="translate(360 944)">'
  +   '<rect x="-58" y="-27" width="116" height="54" rx="10" fill="#12161C" stroke="#A6CE39" stroke-width="3"/>'
  +   '<path d="M-40 13 L-27 -17 L-14 13" stroke="#F4F4F1" stroke-width="7" fill="none" stroke-linecap="round" stroke-linejoin="round"/>'
  +   '<rect x="-37" y="-4" width="21" height="10" rx="5" fill="#0D1117" stroke="#F4F4F1" stroke-width="1.8"/>'
  +   '<circle cx="-26.5" cy="1" r="2.8" fill="#A6CE39"/>'
  +   '<text x="17" y="12" font-family="Barlow Condensed,Impact,sans-serif" font-size="36" font-weight="800" fill="#F4F4F1" text-anchor="middle">F</text>'
  +   '<text x="45" y="12" font-family="Barlow Condensed,Impact,sans-serif" font-size="36" font-weight="800" fill="#F4F4F1" text-anchor="middle">C</text>'
  + '</g>'
  /* tool belt */
  + '<rect x="246" y="964" width="228" height="38" rx="9" fill="url(#afcLth)" stroke="#14181F" stroke-width="5"/>'
  + '<g stroke="#E6C77A" stroke-width="2.6" stroke-dasharray="8 6" opacity=".8"><path d="M252 973 h216 M252 994 h216"/></g>'
  + '<rect x="330" y="958" width="56" height="50" rx="10" fill="url(#afcChr)" stroke="#14181F" stroke-width="5"/>'
  + '<rect x="341" y="969" width="34" height="28" rx="5" fill="#12161C" opacity=".6"/>'
  + '<circle cx="358" cy="983" r="4.5" fill="#A6CE39"/>'
  + '<path d="M230 994 h50 v48 q0 12 -12 12 h-26 q-12 0 -12 -12 z" fill="#6B4529" stroke="#14181F" stroke-width="5" stroke-linejoin="round"/>'
  + '<rect x="452" y="994" width="50" height="32" rx="10" fill="#A6CE39" stroke="#37460F" stroke-width="5"/>'
  + '<path d="M466 1024 l-5 30 h23 l-3 -30 z" fill="#1C222B"/>'
  /* arms */
  + '<path d="M282 876 C 248 892 230 922 226 950" stroke="#14181F" stroke-width="24" fill="none" stroke-linecap="round"/>'
  + '<path d="M282 876 C 248 892 230 922 226 950" stroke="#F4F4F1" stroke-width="14" fill="none" stroke-linecap="round"/>'
  + '<circle cx="224" cy="960" r="19" fill="#C8905A" stroke="#14181F" stroke-width="5"/>'
  + '<g class="afcb-wave">'
  +   '<path d="M440 870 C 474 858 494 832 498 806" stroke="#14181F" stroke-width="24" fill="none" stroke-linecap="round"/>'
  +   '<path d="M440 870 C 474 858 494 832 498 806" stroke="#F4F4F1" stroke-width="14" fill="none" stroke-linecap="round"/>'
  +   '<circle cx="500" cy="796" r="20" fill="#C8905A" stroke="#14181F" stroke-width="5"/>'
  + '</g>'
  /* graduated blade neck + the big head */
  + '<rect x="336" y="700" width="48" height="140" fill="#F0BE22" stroke="#14181F" stroke-width="5"/>'
  + '<path d="M343 826 h20 M343 806 h11 M343 786 h11 M343 766 h11 M343 746 h11" stroke="#14181F" stroke-width="3" stroke-linecap="round"/>'
  + '<text x="374" y="832" font-family="monospace" font-size="15" fill="#14181F" text-anchor="middle">1</text>'
  + '<g class="afcb-bobble"><image href="' + src + '" x="40" y="18" width="640" height="690"/></g>'
  + '</svg>';
}
...
+'.afcb-btn{width:92px;height:auto;background:transparent;border:0;padding:0;cursor:pointer;display:block;position:relative}'
+'.afcb-btn img{width:100%;height:auto;display:block;filter:drop-shadow(0 10px 18px rgba(0,0,0,.45))}'
+'.afcb-btn:hover img{filter:drop-shadow(0 14px 22px rgba(0,0,0,.55))}'
+'@media(prefers-reduced-motion:no-preference){.afcb-btn img{animation:afcbNod 3.4s ease-in-out infinite;transform-origin:50% 92%}}'
+'@keyframes afcbNod{0%,100%{transform:rotate(-3deg)}50%{transform:rotate(3deg)}}'
...
+'.afcb-h .av{width:52px;height:52px;flex:0 0 52px;display:grid;place-items:center}'
+'.afcb-h .av img{width:100%;height:auto;display:block}'
...
+'.afcb-hello svg{width:190px;height:auto;margin:0 auto;display:block}'
+'.afcb-hello svg image{image-rendering:auto}'
...
+'@media(max-width:480px){.afcb{right:14px;bottom:14px}.afcb-btn{width:64px;height:64px}'
 +'.afcb-btn{width:76px}.afcb-p{height:min(560px,calc(100vh - 110px));bottom:80px}'
...
function build(){
  ...
  +   '<div class="afcb-h"><span class="av">' + headTag('avImg','main') + '</span>'
  ...
  +   headTag('btnImg','main') + '<span class="afcb-dot">1</span></button>';
  ...
  var panel = w.querySelector('.afcb-p'),
      ...
      btnImg = null, avImg = null;

  /* He changes face by subject: shades for money, level for hard truths,
     grin when the numbers land. Preloaded so the swap never flickers. */
  function setMood(m){
    if (!MOODS[m] || S.mood === m) return;
    S.mood = m;
    if (btnImg) btnImg.src = MOODS[m];
    if (avImg)  avImg.src  = MOODS[m];
  }
  (function preload(){
    var k; for (k in MOODS){ if (Object.prototype.hasOwnProperty.call(MOODS,k)){
      var i = new Image(); i.src = MOODS[k]; } }
  })();
  ...
  btnImg = w.querySelector('.btnImg');
  avImg  = w.querySelector('.avImg');
  ...
  var api = { say: say, chips: chips, go: go, mood: setMood, form: function(){
  ...
  function open(){
    ...
    if (!opened){
      opened = true;
      var hello = el('div', 'afcb-hello', FULL(S.mood));
